...
create procedure add_article(
  ...
  in p_publication_date datetime, in p_content text(60000), in p_language varchar(5), in p_hash char(32)) language sql
  begin
...
DELIMITER $$
create procedure add_article_history(in p_parent_id bigint, in p_date datetime, in p_user varchar(50), in p_str_date varchar(50)) language sql
  begin
